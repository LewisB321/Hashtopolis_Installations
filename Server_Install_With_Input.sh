...
##Asking for the passwords to be used during SQL configuration
echo "What would you like the root SQL password to be?"
echo "The password must contain a number, a special character, a capital letter and have a minimum length of 8"
read rootpass
echo "And what would you like the hashtopolis SQL user password to be?"
echo "The password must contain a number, a special character, a capital letter and have a minimum length of 8"
read hashpass
...
echo "CREATE DATABASE hashtopolis" | mysql -uroot -p"$rootpass"
echo "CREATE USER 'hashtopolis'@'localhost' IDENTIFIED BY '"$hashpass"'" | mysql -uroot -p"$rootpass"
echo "GRANT ALL PRIVILEGES ON hashtopolis.* TO 'hashtopolis'@'localhost'" | mysql -uroot -p"$rootpass"
...
cd /
echo "root sql user pass = "$rootpass"" > passes.txt
echo "hashtopolis sql user pass = "$hashpass"" >> passes.txt

##Cleaning up
apt purge expect -y
rm Server_Install_With_Input.sh
